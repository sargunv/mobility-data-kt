...
public data class GeofencingZones(
    ...
    /**
     * Array of Rule objects defining restrictions that apply globally in all areas as the default
     * restrictions, except where overridden with an explicit geofencing zone.
     */
    @SerialName("global_rules") public val globalRules: List<GeofencingZoneRule>,
) : GbfsFeedData
...
public data class GeofencingZone(
     ...
     *
     * In the event of colliding rules within the same polygon, the earlier rule (in order of the
     * JSON file) takes precedence. In the case of overlapping polygons, the combined set of rules
