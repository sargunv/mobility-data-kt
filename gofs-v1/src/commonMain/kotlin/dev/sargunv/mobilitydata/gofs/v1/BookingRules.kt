...
public data class BookingRule(
    ...
    /**
     * Minimum number of minutes before travel to make the request. Required for same-day booking
     * with advance notice (booking_type=1). Forbidden otherwise.
     */
    @SerialName("prior_notice_duration_min")
    public val priorNoticeDurationMin: WholeMinutes? = null,
    /**
     ...
     */
    @SerialName("prior_notice_duration_max")
    public val priorNoticeDurationMax: WholeMinutes? = null,
    /**
     * Last day before travel to make the booking request (e.g. "Ride must be booked 1 day in
     * advance before 5PM" will be encoded as prior_notice_last_day=1). Required for prior day(s)
     * booking (booking_type=2). Forbidden otherwise.
     */
    @SerialName("prior_notice_last_day") public val priorNoticeLastDay: Int? = null,
    /**
     * Last time on the last day before travel to make the booking request (e.g. "Ride must be
     * booked 1 day in advance before 5PM" will be encoded as prior_notice_last_time=17:00:00).
    ...
    /**
     * Earliest time on the earliest day before travel to make the booking request (e.g. "Ride can
     * be booked at the earliest one week in advance at midnight" will be encoded as
     ...
     * counted. If empty, prior_notice_start_day=2 will be two calendar days in advance. If defined
     * as a calendar_id containing only business days (weekdays without holidays),
     ...
     * Meant to provide minimal information to be transmitted within a user interface about the
